...
function Modal({anime}){
  ...
  const handleStatusChange = async () => {
    if (session) { // Check if there's a session and current is false
      try {
        const query = `
          query Q{
            Media(id:${id}) {
...
<div className="relative w-full h-[120px] lg:w-[200px] lg:h-[308px]   aspect-[2/3]">
  ...
  <div className='flex space-x-2 text-xs'>
    {anime.externalLinks.filter(link => link.icon !== null && link.type === "STREAMING").map((link) => (
      ...
      { success ? 
     (
      <button className={`outline-none text-action`} onClick={handleStatusChange} disabled>
     <svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24"><path fill="currentColor" d="m12 18l-4.2 1.8q-1 .425-1.9-.162T5 17.975V5q0-.825.588-1.412T7 3h10q.825 0 1.413.588T19 5v12.975q0 1.075-.9 1.663t-1.9.162z"></path></svg>
      </button>)
:

                (
                  <button className={`outline-none text-white"`} onClick={handleStatusChange}>
