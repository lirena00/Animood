...
function Card({ anime }) {
  ...
  const onMouseMove = useCallback((e) => {
    ...
    blob.current.animate(
      {
        transform: `translate(${
          clientX - cardRect.left - blobRect.width / 2
        }px, ${clientY - cardRect.top - blobRect.height / 2}px)`,
      },
      {
        duration: 100,
        fill: "forwards",
  ...
  return (
    <>
        ...
        <div className="inner z-1 h-full w-full rounded-lg">
          <div className="card rounded-md h-full w-full overflow-hidden transition-all duration-200">
            <div className="relative transition-all duration- h-full duration-200  ">
              ...
              <div className="font-medium text-sm text-white py-1">{title}</div>
              ...
              <span className="absolute top-2 right-2 px-2 py-1  bg-primary text-gray-400 rounded-md text-xs ">
                EP {anime.episodes}
              </span>
            </div>
          </div>
      ...
      <div
        ...
        } transition-all duration-300 fixed backdrop-blur-lg top-0 left-0 w-full h-full bg-black bg-opacity-50 z-10 flex items-center justify-center`}
      >
        <div
          onClick={handleClickInside}
          className={`w-[95%] lg:w-[60%] transition-all duration-300 ${
